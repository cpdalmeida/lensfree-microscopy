# ANNEX C – PHASE RECOVERY PYTHON CODE FOR MULTISPECTRAL METHOD

'''
*** Multispectral Pythoncode - alignment and phase reconstruction ***
	~ for entire images (all Field of View) ~
'''
...
# Global variables that are used on the click_and_crop function
...
# load_images opens the selected folder and returns a list with all images that it has inside
...
def crop(image):
	clone = image.copy()
	return clone[int(refPt[0][1]/Scale):int(refPt[1][1]/Scale),int(refPt[0][0]/Scale):int(refPt[1][0]/Scale)]
...
refractive_index = 1        # refractive index - medium between sample to the sensor
num_iterations = 10         # number of iterations
deltaz = 50                 # distance between captured holograms

# ------------- Loading images --------------------- #
Original_imgs = load_images(r"C:\Users\Camila\Downloads\leveduras_azul_metileno_exp900")
...
# Create a Trackbar to choose variable values
cv2.createTrackbar('Lambda', window_name, 632, 700, Interactive_propagation)
cv2.createTrackbar('10z', window_name , 8, 28, Interactive_propagation)
# Call the function to initialize
Interactive_propagation(0)
# Wait until user finishes the program
cv2.waitKey()
...
# image to be used as a reference for functions of propagation
img = Original_imgs[0]
[Nx,Ny] = img.shape
...
cv2.imwrite('Amplitude_final.png',rescale_intensity(Amplitude_img))
cv2.imwrite("Phase_final.png",rescale_intensity(Phase_img))
cv2.imwrite("OnFocus.png",rescale_intensity(np.abs(U(Original_imgs[0],z2[0], Lambda))))
